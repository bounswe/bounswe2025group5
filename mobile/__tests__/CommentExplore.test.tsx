...
describe("ExploreScreen Comment Functionalities", () => {
  ...
  beforeEach(() => {
    ...
    (apiRequest as jest.Mock).mockImplementation((url, options) => {
      ...
      if (url.includes("/api/users/") && url.includes("/profile")) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ photoUrl: null }),
        });
      }
  ...
  it("allows deleting a comment", async () => {
    (apiRequest as jest.Mock).mockImplementation((url, options) => {
      ...
      if (
        url.includes("/api/posts/comment/102") &&
  ...
  it("allows editing a comment", async () => {
    ...
    const { getByText, getByLabelText, getByDisplayValue, getAllByText } =
      render(
        <AuthContext.Provider value={mockUser}>
          <ExploreScreen />
        </AuthContext.Provider>
  ...
  it("displays correct texts in Turkish and English", async () => {
    ...
    await waitFor(() =>
      expect(getByPlaceholderText("Yorum ekle...")).toBeTruthy()
    );
    ...
    currentLanguage = "en";
    const {
      getByText: getByTextEn,
      getByPlaceholderText: getByPlaceholderTextEn,
    } = render(
      <AuthContext.Provider value={mockUser}>
        <ExploreScreen />
      </AuthContext.Provider>
    ...
    expect(getByTextEn("Explore")).toBeTruthy();
    ...
    await waitFor(() =>
      expect(getByPlaceholderTextEn("Add a comment...")).toBeTruthy()
    );
  });
});
